...
#include "Spark/types/darrays/const_char_ptr_darray.h"
...
b8 platform_init(
    ...
    if (xcb_connection_has_error(state->connection)) {
        SCRITICAL("Failed to connect to X server via XCB.");
        return false;
    }
    ...
    if (stream_result <= 0) {
        SCRITICAL("An error occurred when flusing the stream: %d", stream_result);
        return false;
    }
...
void platform_shutdown(platform_state_t* plat_state) {
    ...
    while (event != 0) {
        ...
        switch (event->response_type & ~0x80) {
            ...
            case XCB_KEY_RELEASE: {
                ...
                keycode_t key = translate_keycode(key_sym);
            ...
            case XCB_BUTTON_RELEASE: {
                ...
                mouse_button_t mouse_button = MOUSE_BUTTON_MAX;
...
void platform_get_required_extension_names(const char** names, u32 start_index, u32* out_extension_count) {
    names[start_index] = "VK_KHR_xcb_surface";
...
b8 platform_create_vulkan_surface(
    ...
    if (result != VK_SUCCESS) {
        SCRITICAL("Failed to create vulkan xcb surface");
        return false;
    }

    *out_surface = state->surface;
    return true;
}
