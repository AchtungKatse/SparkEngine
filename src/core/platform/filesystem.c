...
b8 filesystem_open(const char* path, file_access_t mode, b8 binary, file_handle_t* out_handle) {
...
b8 filesystem_read_line(file_handle_t* handle, u64 max_length, char** line_buf, u64* out_line_length) {
...
b8 filesystem_read(file_handle_t* handle, u64 data_size, void* out_data, u64* out_bytes_read) {
...
b8 filesystem_read_all_bytes(file_handle_t* handle, u8* out_bytes, u64* out_num_bytes_read) {
    if (handle->handle && out_bytes && out_num_bytes_read) {
        ...
        *out_num_bytes_read = fread(out_bytes, 1, size, (FILE*)handle->handle);
        return *out_num_bytes_read == size;
    }
    return false;
}

b8 filesystem_read_all_text(file_handle_t* handle, char* out_text, u64* out_bytes_read) {
...
b8 filesystem_write(file_handle_t* handle, u64 data_size, const void* data, u64* out_bytes_written) {
    if (handle->handle) {
