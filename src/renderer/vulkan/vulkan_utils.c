...
u32 vulkan_get_memory_index(struct vulkan_context* context, u32 required_memory_types, VkMemoryPropertyFlags required_properties) {
    const VkPhysicalDeviceMemoryProperties* device_properties = &context->physical_device.memory_properties;

    for (u32 i = 0; i < device_properties->memoryTypeCount; i++) {
         b8 is_supported = required_memory_types & (1 << i);
         b8 has_required_properties = (device_properties->memoryTypes[i].propertyFlags & required_properties) == required_properties;

         if (is_supported && has_required_properties) {
             return i;
         }
    }

    SCRITICAL("Failed to find memory type for %x requested memory properties %x\n", required_memory_types, required_properties);
    return -1;
}

void vulkan_image_view_create(vulkan_context_t* context, VkImage image, VkImageViewType view_type, VkFormat format, VkImageAspectFlags aspect_mask, u32 layer_count, u32 mip_levels, VkImageView* out_image_view) {
    ...
    vkCreateImageView(context->logical_device, &create_info, context->allocator, out_image_view);
}
